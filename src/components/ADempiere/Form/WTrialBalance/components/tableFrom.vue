...
export default {
  ...
  methods: {
    ...
    toggleSelection(rows) {
      if (rows) {
        rows.forEach(row => {
          const selectionRow = this.recordsData.find(record => record.accountNo === row.accountNo)
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (values.every(value => typeof value === 'number') && !this.isEmptyValue(this.selection)) {
